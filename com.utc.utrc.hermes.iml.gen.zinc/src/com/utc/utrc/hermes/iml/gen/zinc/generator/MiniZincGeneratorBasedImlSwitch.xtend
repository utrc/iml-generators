...
public class MiniZincGeneratorBasedImlSwitch extends ImlSwitch<String> {
	...
	def public void translateIMLModel2CPModel(String pathIML, String modelName) {
		...
		val model = findModel(allResources, modelName);
		caseModel(model as Model);
	}
	
	def findModel(ResourceSet rs, String modelName) {
		for (Resource res : rs.resources) {
			if (!res.contents.nullOrEmpty) {
				val model = res.contents.get(0)
				if (model instanceof Model && (model as Model).name == modelName) {
